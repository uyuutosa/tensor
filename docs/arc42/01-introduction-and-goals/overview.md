...
| 0.3.0   | 2026-05-13 | **Phase 6 (Python SDK via nanobind) shipped end-to-end** across 17 PRs (`#95`–`#117`) in two days. [ADR-0018](../09-decisions/0018-phase-6-python-sdk-entry-via-nanobind.md) anchors the entry decisions (nanobind + in-tree `python/` + scikit-build-core + `tensor-named-axis` PyPI name + minimum-viable surface). [ADR-0019](../09-decisions/0019-phase-6-5-runtime-backend-selection-via-extras.md) decides Phase 6.5's `set_backend()` packaging trade-off — **PEP-508 extras** over a fat wheel (~5 MB educational default install; Dawn opt-in via `[webgpu]`). The system identity in §1 expands: `tensor` is now a **C++ Domain + Python SDK (DrivingAdapter)** rather than C++ only. G-1 + G-3 still hold for the C++ surface; a new G-8 captures the Python SDK as a first-class entry to the same Domain. Success criteria extend with Phase 6 + 6.5 milestones. §7 cross-references list extended to ADR-0018, ADR-0019, the [Phase 6 retrospective](../../reports/2026-05-13_phase-6-python-sdk-retrospective.md), and the eighth detailed-design instance ([`python-sdk-binding-surface.md`](../../detailed-design/python-sdk-binding-surface.md)). |
...
- **G-9 — Python is a first-class entry to the same Domain ([ADR-0018](../09-decisions/0018-phase-6-python-sdk-entry-via-nanobind.md)).** The Python SDK (`tensor-named-axis` on PyPI) is a DrivingAdapter ([ADR-0009](../09-decisions/0009-adopt-ddd-ubiquitous-language-and-hexagonal-lite.md)) — not a re-implementation in Python. Every Python entry point calls into the same C++ Domain (`tensor::core` + `tensor::autograd` + `tensor::tex`) the C++ tutorials exercise; nanobind wraps the boundary. Surfaces parity: `DynamicTensor` + arithmetic broadcast + `contract` + NumPy interop + `autograd.DynamicVariable` (+ `sin`/`cos`/`sqrt`/`__truediv__`/`reduce_along_label`) + `tex.parse` + `tex.Evaluator`. The wedge that motivates this Goal is captured in the [2026-05-12 landscape recheck report §A.5](../../reports/2026-05-12_landscape-recheck-and-adversarial-review.md): *no first-class named-axis tensor exists in production Python ML* — Phase 6 opens that gap.
