...
- [x] `tutorials/06_webgpu-acceleration.ipynb` exists and walks the reader through Phase 3 end-to-end. *(Shipped in Option 3 "design walkthrough" form per Phase 4 release rehearsal #48; P3.M3.2 element-wise dispatch wiring shipped subsequently — the 8 element-wise WGSL kernels now run on real GPU and are cross-validated against the reference backend within `1e-5` for `float`. GEMM dispatch wiring follows in P3.M4.2.)*
