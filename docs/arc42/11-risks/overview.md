...
| -- | ---- | -------- | ------------- | ---------- |
| R-S1 | **gpu.cpp goes dormant** (single-maintainer; bus factor 1; last release v0.2.0 Aug 2024). | 🔴 High | Soft-maintenance; commit cadence ~ one cluster every few months. | Vendored under `third_party/gpu_cpp/gpu.hpp@0.2.0` with `VENDORED_FROM` record (PR #41); `tools/check-vendored.sh` lint gates `build-and-test`. Re-vendor procedure documented. |
