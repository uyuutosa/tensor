...
| `python/`                | **DrivingAdapter** | 6 🚧 (M1 shipped) | Python SDK via nanobind. M1 scaffolds `pyproject.toml` + scikit-build-core + `_tensor_native` nanobind module with a smoke `hello()` surface. M2..M6 add `DynamicTensor` + arithmetic / `contract` + NumPy interop / autograd / `tex.Evaluator` / backend selection per the [Phase 6 impl-plan](../../impl-plans/2026-05-12_phase-6-python-sdk.md). | [0018](../09-decisions/0018-phase-6-python-sdk-entry-via-nanobind.md), [0009](../09-decisions/0009-adopt-ddd-ubiquitous-language-and-hexagonal-lite.md) |
