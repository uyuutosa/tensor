...
**Shipped / in-flight as of 2026-05-11** (after PR #1–#28): `tensor::core` ✅ (incl. `LabelTag` + `TypedTensor` compile-time path); `tensor::tex` ✅ (parser + `to_latex` + `_tex` UDL + Evaluator end-to-end); `tensor::autograd` ✅ (Phase 2 full: MVP, activations, broadcast backward, contraction, `zero_grad`, `sgd_update`); two backend adapters ✅ (`reference`, `eigen`); tutorials 00 / 05 / 07 / 08 ✅; Jupyter Book scaffold ✅ ([`book/`](../../../book/)). **In flight / planned**: WebGPU adapter (Phase 3 plan dated 2026-05-11 — [`../../impl-plans/2026-05-11_phase-3-webgpu.md`](../../impl-plans/2026-05-11_phase-3-webgpu.md)); mdspan polyfill restore, xeus-cling notebook CI, LyX export module (Phase 1.5 backlog); `0.1.0` release tag (Phase 4 close).
