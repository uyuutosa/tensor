...
- **P3.M3.2 — Dispatch wiring (deferred).** Replaces each `webgpu::Backend::{add,sub,mul,div,exp,log,relu,neg}` stub delegation with the gpu.cpp dispatch sequence described in the detailed-design doc §3 (binary) + §3.1 (unary). Preconditions: vcpkg baseline bumped to include `dawn@20260410.140140`+ (PR #47 forward-declared the `webgpu` manifest feature so the dep shape is discoverable, but kept the baseline pinned); self-hosted GitHub Actions runner with a Dawn-compatible GPU exists; ADR-0012's compile-only-CI policy upgrades to "compile-only on generic CI + numerical-agreement on self-hosted runner".

  Note on the baseline bump: the vcpkg `2026.04.27` release (commit `56bb2411609227288b70117ead2c47585ba07713`) is the first stable tag that includes `dawn@20260410.140140`. Bumping to it also pulls Eigen 3.4 → 5.0.1, which is a major-version change that warrants its own focused CI iteration. The P3.M3.2 PR therefore bundles the baseline bump with Dawn linkage + Eigen-5 compatibility verification.
