...
- **P3.M3.2 — Dispatch wiring (shipped).** Replaces each `webgpu::Backend::{add,sub,mul,div,exp,log,relu,neg}` stub delegation with real Dawn dispatch using **Dawn's own `webgpu_cpp.h`** wrapper (per [ADR-0016](../arc42/09-decisions/0016-substrate-refinement-drop-gpu-cpp-talk-to-dawn-directly.md), superseding ADR-0014's gpu.cpp wrapper choice). Float (f32) goes through Dawn; other dtypes delegate to reference per ADR-0012 § f32-only MVP. `CMakeLists.txt` adds `find_package(Dawn CONFIG REQUIRED)` + `target_link_libraries(... dawn::webgpu_dawn)` gated on `TENSOR_KERNEL_BACKEND=webgpu`. The project's vcpkg manifest baseline is unchanged; consumers override via `-DCMAKE_PREFIX_PATH=<vcpkg>/x64-linux` to discover the locally-installed Dawn port (`dawn[core,vulkan]`). **Verified locally on RTX 3090 + Dawn 2026-04 + Vulkan: 226 webgpu-specific test assertions pass against `reference::Backend` within `1e-5` for `float`.**

  Detail-design implementation lives in [`include/tensor/core/backend/webgpu_detail/`](../../include/tensor/core/backend/webgpu_detail/) — `context.hpp` manages thread-local instance/adapter/device/queue lifetime; `dispatch.hpp` compiles WGSL + runs the compute pass + reads results via a staging buffer. The `binary_op` / `unary_op` template helpers in `webgpu.hpp` share the dispatch shape across 8 operators.
