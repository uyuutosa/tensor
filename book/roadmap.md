...
- **Phase 6.5 — runtime `set_backend()`** (deferred out of P6.M6 per the Phase 6 retrospective). Selects the linked `KernelBackend` adapter (`reference` / `eigen` / `webgpu`) at runtime from Python. Wheel-packaging trade-off — PEP-508 extras (`tensor[eigen]` / `tensor[webgpu]`, one backend per wheel) vs single fat wheel (~30 MB) bundling all three — is the open design choice.
...
- **Live decisions**: [`docs/arc42/09-decisions/`](https://github.com/uyuutosa/tensor/tree/develop/docs/arc42/09-decisions) (18 ADRs as of 2026-05-13; ADR-0013 superseded by ADR-0015; ADR-0014 §Decision Outcome point 2 refined by ADR-0016; ADR-0015 §Compliance bullet 3 refined by ADR-0017; ADR-0018 anchors Phase 6).
